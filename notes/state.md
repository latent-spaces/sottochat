...
what's wired: multi-source tailer (cc cli + claude.app + **codex rollouts**, recursive fs.watch + per-tick poll, lineage collapse + temp-folder filter) · **codex JSONL parser** (`src/codex-jsonl.ts` — stateful, ports vendor/abtop/src/collector/codex.rs schema; codex events flow through turns/observer/chat-agent unchanged) · chat-session display names (server resolves `cut-the-cake-chat-<sha1[:12]>` slugs back to upstream and emits `displayName: "<project> · chat"`) · **abtop-style PID-driven session discovery in shadow mode** (process-discovery + claude-discovery + codex-discovery + registry; runs every 2s alongside the legacy tailer, exposed at `/diag/discovery` for parity verification, doesn't drive the inbox yet) · multi-session inbox · diff-rendered sidebar with magicui-style enter/exit/update pulse + FLIP layout (transform-leak fix) · **inbox separator + grouping**: user-driven sessions on top, `internal · sdk subprocesses` divider, then observer + chat-agent + scriptifier threads · single-loop sonnet observer (auto-respawn) · chat-agent host (sonnet, one persistent subprocess per upstream sessionKey, tools disabled) · POST `/chat/send` + ws chat:chunk/chat:status broadcasts · auto-send on observer open=true with 5-min cooldown + global toggle (default off) · chat-thread block in detail pane (markdown, "auto" badge for server-fired chunks) · **chat-thread collapses to last 2 messages by default** (per-session "show N earlier" toggle, auto-collapses on send) · charts above the conversation, panel-styled, capped to last 5 turns · **animated charts-band expand/collapse with first-look intro** (bars stagger-grow from the floor, deflate end-to-start, band height folds, toggle pulses) · **video-pane** (parallel-modality alternative to reading the conversation strip — per-turn karaoke captions + marker overlays + TTS audio + interactive bookmarks + mp4 export). **scriptifier** subprocess pool: one sonnet sdk subprocess per `(scriptStyle, markerVocab)` combination, lazy-spawned on first use, sandbox cwd `~/.cut-the-cake/scriptifier/<style>--<vocab>/`. emits `{turnId, beats:[{text, marker?, emphasis?}]}` with defensive output caps (≤30 beats, ≤240 chars/beat, ≤5 emphasis/beat, ≤40 char/emphasis, ≤10 scripts/batch, dedupe by turnId) and prompt-side input caps (≤60 events sampled head-30 + tail-30, ≤12k char body). markers: design vocab (INSIGHT/BE_CAREFUL/STEP/NOTE) or story vocab (PUNCHLINE/GOTCHA/PIVOT/ASIDE) — same semantic intent, different prose energy. **seven script styles**: default (current), cinematic (documentary gravitas), tldr (3-5 ultra-terse beats), deep-dive (10-25 specific technical beats), comedic (setup→twist→payoff), noir (terse atmospheric), kids (concrete metaphors). **per-session voice**: af_heart "warm" / am_michael "calm" / bf_emma "crisp", each with character tagline. server runs `npx hyperframes tts` (kokoro-82m local) + `npx hyperframes transcribe` (whisper small.en local) per-job, transcribe in temp dir to avoid `transcript.json` race. sha256-cached at `~/.cut-the-cake/tts-cache/<hash>.{wav,words.json,txt}`, served via `GET /tts/<hash>.wav` (64-hex+`.wav` path guard). **mp4 export** via `src/hyperframes-export.ts`: scaffolds a real hyperframes composition (1280x720 lyric-scroll layout, per-word soft-pink highlight, banner-style marker overlays, audio at track 0), runs `npx hyperframes render --quality draft --workers auto`, caches at `~/.cut-the-cake/exports/<hash>/{index.html,narration.wav,output.mp4,render.log}`, serves `/export/<hash>.mp4`. e2e verified: 22.2s render of a 41s wav. client renders a `.video-pane` block between charts-band and conversation with a 5-picker head row (voice → script-style → marker-vocab → marker-style → tour) plus an export button. scrolling beat stack, rAF-driven word-level karaoke via `audio.currentTime`, **six marker treatments** (pill / banner / corner / stamp / ticker / halo) selectable per-client at runtime, **tour-mode** button cycles all six every 4s for a single-playback design space tour. per-beat **bookmarks** with seek-chip row above the controls (clickable `📑 #N` jumps audio to beat startS). **frosted play button + sprinkle burst on word advance** ties the pane back to the strawberry/dessert idiom. broadcasts `script:beats` / `script:ready` / `script:error`, `scriptstyle:setting`, `markervocab:setting`, `voice:setting`, `bookmark:setting`, `export:rendering` / `export:ready` / `export:error`. soft pink visual system (full strawberry/dessert redesign, six-hue per-session palette) · sticky frosted nav with github pill + auto-break-down toggle · sidebar/detail visual separation · svg dripping-frosting cap on bar plots (color-mix lightened, only on tall bars) · hover-burst radial sprinkle spray out of frosted bars · brand jump-roll on the cake logo · **wandering cake-perch with 15 mild reactions + 4 over-the-top specials (1/20 chance) + pester-driven wander** (2-5 hovers in a 30s window relocates to a fresh anchor) · undraggable mascot images · palette-unified mascot trio (`mascot-uni-1/2/3.svg`) in the wandering pool · sidebar selected-card mascot with hover y-flip + fade in/out · DESIGN.md regenerated from current code (tagged `pre-design-md-refresh` / `post-design-md-refresh`).
...
| `META_TTS_VOICE`               | `af_heart`           | default kokoro voice for `npx hyperframes tts`. per-session overrides via the voice picker write to a server-side Map; this is the fallback for sessions without an override. |
| `META_SCRIPT_STYLE`             | `default`            | starting scriptifier prompt preset — one of `default / cinematic / tldr / deep-dive / comedic / noir / kids`. runtime-mutable via the picker / POST `/debug/script-style`. |
| `META_MARKER_VOCAB`             | `design`             | starting marker vocabulary — `design` (INSIGHT/BE_CAREFUL/STEP/NOTE) or `story` (PUNCHLINE/GOTCHA/PIVOT/ASIDE). runtime-mutable via the picker / POST `/debug/marker-vocab`. |
| `META_DEBUG`                    | unset                | set `1` to enable `POST /debug/inject-script` (csrf-guarded; only allows localhost Origin). other `/debug/*` runtime-feature routes are always on. |
...
- commit `75b13d6` — video-pane v1 (initial build). new `src/scriptifier.ts` (single sonnet sdk subprocess; tools all disallowed; cwd `~/.cut-the-cake/scriptifier/`; emits `{scripts:[{turnId, beats:[{text, marker?, emphasis?}]}]}`; markers `INSIGHT|BE_CAREFUL|STEP|NOTE`). new `src/tts.ts` (wraps `npx hyperframes tts` + `npx hyperframes transcribe`; sha256-keyed cache; default voice `af_heart`; ffprobe for duration). server hookup: `SessionState.scripts` lifecycle `drafting→rendering→ready|error`; ws msgs `script:beats` / `script:ready` / `script:error`; `GET /tts/<hash>.wav`. `public/index.html` adds the `.video-pane` block with rAF-driven word-level karaoke highlight, fixed cross-session marker colors, 44px play/pause + scrub + mm:ss. e2e verified live (9 beats / 2 markers / 41.4s wav / 98 word timings).
- commit `2305ab6` — `/debug/inject-script` (gated behind `META_DEBUG=1`) for visual verification without waiting for a real turn to close. self-bootstrapping fixture using the cached cafebabe demo. tag `video-pane-v1` at `75b13d6`.
- commit `ecb9108` — codex-driven fixes (high+medium+low). `tts.ts` per-job temp dir to avoid `transcript.json` race. `/debug/inject-script` CSRF guard (Origin must be localhost) + valid JSON + explicit sessionKey. `hyperframes@^0.5.3` pinned. `SessionState.scripts` ring-buffered to 12 entries via `pruneScripts()`. `closedTs` added to `ScriptPayload` + client picks active by `closedTs` desc (late tts can't hijack newer turns). `/tts/<hash>.wav` regex tightened to `^[a-f0-9]{64}\.wav$`. sidebar internal-grouping recognises `cut-the-cake-scriptifier` slugs. observer.ts EOF blank line trimmed.
- commit `592d19f` — H₂ marker style variants. picker (`?marker=…` URL or click-toggle) selects pill / banner / corner. fixed cross-session marker colors per type. Web Audio cue (200Hz @ 8% volume) on corner-style marker entry.
- commit `48c788b` — H₃ scriptifier prompt presets. `default | cinematic | tldr | deep-dive`. lazy per-style sdk subprocess at `~/.cut-the-cake/scriptifier/<style>/`. `POST /debug/script-style` runtime-mutable; broadcasts `scriptstyle:setting`.
- commit `226aad6` — H₄ per-session voice picker. `voicesBySession` Map + `voiceFor(key)` helper. `POST /debug/voice` + `/debug/regen-script` (regen UI button deferred). `voice` field added to snapshot payload.
- commit `1a70d27` — H₅ scriptifier prompts: comedic / noir / kids. brings the total to seven styles.
- commit `e189b1a` — `notes/creative-review.md`: opinionated 8-axis self-critique with concrete alternatives.
- commit `2b2b7d6` — H₆ mp4 export + 3 more marker treatments (stamp / ticker / halo) + script-style picker html residual. new `src/hyperframes-export.ts` (623 lines) builds a 1280x720 lyric-scroll composition with per-word highlight tweens + banner-style marker overlays. cache at `~/.cut-the-cake/exports/<hash>/{index.html,narration.wav,output.mp4,render.log}`. `POST /debug/export-script` + `GET /export/<hash>.mp4`. ws msgs `export:rendering` / `export:ready` / `export:error`. e2e verified live: 22.2s render of 41s demo wav, 2 MB output, served clean.
- commit `7912f10` — `notes/overnight-variations.md`: morning tour doc with how-to-see-it + endpoint table.
- commit `419e621` — H₇ scriptifier defensive caps. output: ≤30 beats, ≤240 chars/beat, ≤5 emphasis/beat, ≤40 chars/emphasis (drop, don't truncate), ≤10 scripts/batch with first-wins dedupe. input: ≤60 events sampled head-30+tail-30, ≤12k char body with middle-truncation.
- commit `010acff` — H₈ video-pane visual identity. frosted play button (mask-image: frosting-new.svg, color-mix lighter). sprinkle burst on word advance via the existing `spawnFlyer` helper, 320ms cooldown so rapid stretches don't strobe. tag `overnight-variations-v1` here.
- commit `e2218d2` — `notes/overnight-variations.md`: removed the "mp4 render hasn't been triggered" caveat after live verification.
- commit `8d217a8` — H₉ alternative marker vocabulary. `MarkerVocab = "design" | "story"` with `MARKER_VOCAB_DESIGN` / `MARKER_VOCAB_STORY` prompt blocks. `buildSystemIntro(style, vocab)` swaps the marker block + word-boundary renames the prose's existing `INSIGHT/BE_CAREFUL/STEP/NOTE` mentions. all 14 (style × vocab) subprocesses lazy-spawn. server `activeMarkerVocab` global + `POST /debug/marker-vocab` + hello payload field. client `markerVocab` global, `MARKER_DISPLAY` translation map, picker between scriptStyle + markerStyle pickers.
- commit `3e2f448` — `notes/overnight-variations.md`: tour catches up with R + 5-picker row.
- commit `d52fd52` — H₁₀ per-turn beat bookmarks. server `bookmarks: Map<sessionKey, Map<turnId, Set<number>>>`. `POST /debug/bookmark {action: add|remove|toggle}` + `/debug/bookmarks/clear`. ribbon-icon button on each beat (visible while .current or on hover). `.video-bookmarks` chip row above the controls; click chip seeks audio to `beatBoundaries[idx].startS`.
- commit `5abd497` — `notes/overnight-variations.md`: live-verified endpoints table.
- commit `4b3aa16` — H₄′ voice picker character taglines ("the friend who calls at 11pm" / "the voice you find when you slow down" / "your professor before tenure"). dropdown options become two-line.
- commit `e96cb92` — H₁₁ marker-tour button. cycles all six marker treatments every 4s during a single playback so the user can see the full design space without manual clicking. starts at the user's current style, restores it on stop. dashed pink when idle, full accent + soft pulse animation while active. tag `overnight-final` here.
- commit `c9a78ed` — `notes/overnight-variations.md`: tour mode + V commit.
...
| H₁    | codex review fixes: tts transcribe race, /debug/inject-script CSRF, hyperframes pinned, scripts ring-buffered, /tts hash tightened, sidebar grouping | done · committed (`ecb9108`) |
| H₂    | marker style variants — pill / banner / corner (URL ?marker=… + localStorage picker) | done · committed (`592d19f`) |
| H₃    | scriptifier prompt presets — default / cinematic / tldr / deep-dive (per-style sdk subprocesses) | done · committed (`48c788b`) |
| H₄    | per-session voice picker — warm / calm / crisp + character taglines | done · committed (`226aad6`, taglines `4b3aa16`) |
| H₅    | 3 more scriptifier prompts — comedic / noir / kids | done · committed (`1a70d27`) |
| H₆    | mp4 export via real hyperframes composition + 3 more marker treatments (stamp / ticker / halo) | done · committed (`2b2b7d6`); render verified live at 22.2s for the demo |
| H₇    | scriptifier defensive caps — beats/emphasis/scripts output + events/body input | done · committed (`419e621`) |
| H₈    | video-pane visual identity — frosted play button + sprinkle burst on word advance | done · committed (`010acff`) |
| H₉    | alternative marker vocabulary — design ↔ story (PUNCHLINE/GOTCHA/PIVOT/ASIDE) | done · committed (`8d217a8`) |
| H₁₀   | per-turn beat bookmarks — click any beat, seek-chip row above controls | done · committed (`d52fd52`) |
| H₁₁   | marker-tour button — cycles all 6 treatments every 4s during one playback | done · committed (`e96cb92`) |

**current head:** `c9a78ed`. tag `overnight-final` at `e96cb92`. tag `overnight-variations-v1` at `010acff` (mid-run anchor). 20 commits since `pre-registry-flip`. tour at [notes/overnight-variations.md](overnight-variations.md), opinionated critique at [notes/creative-review.md](creative-review.md). short version: the video-pane now carries a 5-picker head row (voice → script-style → marker-vocab → marker-style → tour) plus mp4 export and per-beat bookmarks. 3 voices · 7 scriptifier prompts · 2 marker vocabularies · 6 marker treatments · real hyperframes mp4 render path. **visual layer still unverified in a browser** — chrome-devtools-mcp wedged the whole run; first eyes-on is the morning pass.
...
src/scriptifier.ts               POOL of sonnet sdk subprocesses, one per (scriptStyle, markerVocab)
                                 combination — all tools disallowed, lazy-spawned on first use, sandbox
                                 cwd ~/.cut-the-cake/scriptifier/<style>--<vocab>/. seven styles
                                 (default | cinematic | tldr | deep-dive | comedic | noir | kids) and
                                 two vocabularies (design = INSIGHT/BE_CAREFUL/STEP/NOTE | story =
                                 PUNCHLINE/GOTCHA/PIVOT/ASIDE) — fourteen possible subprocesses, lazy.
                                 default batchMs=2000 so video lands ~2s after a turn closes. batch
                                 body embeds the full event transcript with caps (≤60 events sampled
                                 head-30+tail-30, ≤2000 chars/block, ≤12k char body). per-style
                                 preamble + COMMON_TAIL (response shape + marker rules) → systemIntro.
                                 buildSystemIntro(style, vocab) swaps the marker block + word-boundary
                                 renames the prose for the story vocab. parser caps model output:
                                 ≤30 beats/script, ≤240 chars/beat, ≤5 emphasis/beat, ≤40 chars/
                                 emphasis (drop, don't truncate), ≤10 scripts/batch with first-wins
                                 dedupe by turnId. exports: ScriptStyle, MarkerVocab, ScriptResult,
                                 buildTurnFeed (re-export from observer with events[] populated),
                                 startScriptifier({onScript, model, batchMs}) returning
                                 feed(t, style?, vocab?) + stop().
src/tts.ts                       wraps `npx hyperframes tts` + `npx hyperframes transcribe` + ffprobe.
                                 ...
                                 <hash>.words.json exist. transcribe runs in a per-job temp dir
                                 (~/.cut-the-cake/tts-cache/.tmp-<hash>/) so concurrent transcribes
                                 don't clobber the shared transcript.json. in-memory in-flight
                                 Map<hash, Promise> dedupes concurrent calls. default voice af_heart
                                 (kokoro-82m local, verified); per-session overrides selectable from
                                 voicesBySession via voiceFor(sessionKey). returns {hash, audioPath,
                                 durationS, words: WordTiming[]}.
src/hyperframes-export.ts        builds a real hyperframes composition (1280x720 lyric-scroll
                                 layout, per-word soft-pink highlight tweens, banner-style marker
                                 overlays, audio at track 0) from a ScriptPayload. cache key
                                 sha256({beats, voice, audioHash, version:"v1"}) — version literal
                                 lets us bust every cached mp4 with one constant flip. cache dir
                                 ~/.cut-the-cake/exports/<hash>/{index.html,narration.wav,output.mp4,
                                 render.log}. on miss: scaffold + copy wav as narration.wav, run
                                 npx hyperframes lint (warn-only) then npx hyperframes render
                                 --quality draft --workers auto. on hit: ffprobe + statSync only.
                                 exports exportToMp4, exportMp4Path, exportCacheDir.
...
public/index.html                spa: sticky frosted nav (logo with brand-jump-roll + auto break-down
                                 ...
                                 conversation: 5-picker head row (voice with character taglines →
                                 script-style → marker-vocab → marker-style + tour-mode button →
                                 mp4 export), scrolling beat stack with current-focused / past-dim /
                                 future-dimmer states, rAF-driven word-level karaoke highlight via
                                 audio.currentTime, six marker treatments (pill / banner / corner /
                                 stamp / ticker / halo) with fixed cross-session colors mapped to
                                 both vocab tokens, frosted play button (mask-image: frosting-new.svg)
                                 + sprinkle burst on word advance via spawnFlyer (320ms cooldown),
                                 per-beat bookmarks (ribbon icon + .video-bookmarks chip row above
                                 controls; click chip seeks audio to beat startS), 44px play/pause +
                                 scrub + mm:ss time, runtime cache `videoRuntime: Map<sessionKey,
                                 {audio, beatBoundaries, rafId, lastBannerBeatIdx, lastCornerBeatIdx,
                                 lastStampBeatIdx, lastTickerBeatIdx, lastHaloBeatIdx,
                                 lastWordBurstTs}>` keyed on (turnId, audioUrl) so refresh ticks
                                 reuse the audio element instead of remounting · chat-thread block +
                                 chat input · wandering cake-perch with 15 mild reactions + 4 over-
                                 the-top specials (1/20 chance) + pester-driven wander (2-5 hovers
                                 in a 30s window relocates) · undraggable mascot images.
...
- ✅ video-pane build (3-hour autonomous run; pre-anchor `pre-registry-flip` at `a1044a9`) — initial scriptifier + tts module + .video-pane karaoke renderer, e2e verified live. tag `video-pane-v1` at `75b13d6`.
- ✅ overnight variations (further autonomous run; mid-anchor `overnight-variations-v1` at `010acff`, end-anchor `overnight-final` at `e96cb92`). twenty commits stacked on top of video-pane v1. covers: codex-driven fixes (race + CSRF + ring-buffer + ordering), six marker treatments (pill/banner/corner/stamp/ticker/halo), seven scriptifier prompt presets (default/cinematic/tldr/deep-dive/comedic/noir/kids), per-session voice picker with character taglines, alternative marker vocabulary (design ↔ story = INSIGHT/BE_CAREFUL/STEP/NOTE ↔ PUNCHLINE/GOTCHA/PIVOT/ASIDE), real hyperframes mp4 export (verified 22.2s render of 41s wav), defensive caps on scriptifier output + input, per-turn beat bookmarks with seek-chip row, frosted play button + sprinkle burst on word advance, marker-tour button cycling all six treatments. opinionated 8-axis self-critique at `notes/creative-review.md`, full tour at `notes/overnight-variations.md`. visual layer in a browser still NOT verified — chrome-devtools-mcp wedged the whole run.

---

**resume-here queue** (in priority order):

1. **visual verification of the video-pane** (now covering twenty commits of variations). open http://localhost:3737/, click into the `claude-meta` workspace card, confirm:
   - the demo script (already injected with bookmarks at beats 0/3/7) renders between charts-band and conversation.
   - the 5-picker head row reads voice → script-style → marker-vocab → marker-style + tour-mode + mp4 export, in that order.
   - all six marker treatments (pill / banner / corner / stamp / ticker / halo) animate cleanly when the karaoke advances into the marked beats. tour-mode button cycles them every 4s and pulses while active.
   - voice picker dropdown shows the three options with character taglines. swapping voice doesn't crash anything (only affects future turns; existing audio stays).
   - script-style picker writes to `/debug/script-style` cleanly.
   - marker-vocab picker swaps INSIGHT-family ↔ PUNCHLINE-family labels in the UI; chip colors stay aligned to the same hue per pair.
   - per-beat bookmark icons appear on hover or on the current beat. clicking the seek-chips above the controls jumps the audio.
   - frosted play button reads as a frosting cap with the strawberry color-mix lighter. word advance fires a small sprinkle burst (max 1 per 320ms).
   - mp4 export button kicks off a `rendering 0:NN` ticker and resolves to `download mp4 · NNNkb` (~20-30s for the demo).
   - cascade blur-fade picks up the `.video-pane` region.
   - reduced-motion path doesn't break (banner becomes fade-only, sprinkle burst skipped, tour-mode pulse off).
   likely small CSS/animation tweaks land in this pass. open follow-up TODOs not yet shipped: older-script picker (multiple turns per session — `activeScriptByKey` is wired but no UI yet); auto-play once `script:ready` arrives if the session is currently visible AND there's been a user gesture; multi-word emphasis matching (today's tokenizer can't match `["edge case"]`); fallback word-distribution recompute on `loadedmetadata` when timings absent; mobile/narrow stacking for the controls row; `aria-live="polite"` on the stage; pause-all on ws close; client-side regen-script button (server endpoint already exists).
